...
${{buildRankingRowsHtml(rows, nameLabel)}}
    ...
    function updateMetricCard(selector, value, subtitle) {{
      document.querySelectorAll(selector).forEach((card) => {{
        ...
        const subtitleElement = card.querySelector(".metric-sub");
        if (valueElement && !card.dataset.defaultValue) {{
          card.dataset.defaultValue = valueElement.innerHTML;
        }}
        if (subtitleElement && !card.dataset.defaultSubtitle) {{
          card.dataset.defaultSubtitle = subtitleElement.innerHTML;
        }}
        if (valueElement) {{
          valueElement.textContent = value === null ? "" : value;
          if (value === null) valueElement.innerHTML = card.dataset.defaultValue || "";
        }}
        if (subtitleElement) {{
          subtitleElement.textContent = subtitle === null ? "" : subtitle;
          if (subtitle === null) subtitleElement.innerHTML = card.dataset.defaultSubtitle || "";
        }}
      }});
    }}
    ...
    function updateMainMetrics(selected, totalEntregas, totalPeso, selectedNames) {{
      ...
      const selectedNameList = Array.from(selectedNames.values());
      const subtitle = selectedNameList.length === 1
        ? `Dados de ${{selectedNameList[0]}}`
        : "Dados dos selecionados";
      updateMetricCard(".metric-card.metric-total", formatQuantityValue(totalEntregas), subtitle);
      updateMetricCard(".metric-card.metric-primary", formatWeight(totalPeso), subtitle);
    }}
    ...
    function updateSelectedMetric(selected) {{
      ...
      document.querySelectorAll('[data-comparison-source="colaboradores"] tbody tr[data-colaborador-key]').forEach((row) => {{
        ...
        selectedNames.set(row.dataset.colaboradorKey, row.dataset.colaboradorName || row.dataset.colaboradorKey);
      }});

      updateMainMetrics(selected, totalEntregas, totalPeso, selectedNames);
